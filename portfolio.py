...
class Portfolio:
    def setup_layout(curr_user):
        ...
        if len(curr_user.portfolio) == 0:
            coin_display += ([[sg.Frame('',[[sg.Text("Once you buy a coin, it will appear here.")]],visible=True,k='-COINBOXTOOLTIP-')]])
        for i in range(len(curr_user.portfolio)):
    ...
    def update_layout(curr_user, window, events, values):
        for i in range(len(curr_user.portfolio)):
            ...
            print(curr_user.portfolio[i].quantity)
